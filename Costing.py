...
def Costing(Process_specs, Process_param, Membrane_1, Membrane_2): #process specs is dictionary with mem1, mem2, Pre_cond1, Pre_cond2, Compressor_train, Process_param
    ...
    Process_Contigency = 0.30 # 30% - contingency for process design for TRL-7
    ...
    TDC = DEC * Process_Contigency # Total direct cost with process contingency
    ...
    Penalty_purity = 1e7 * (Process_param["Target_Purity"] - Process_specs["Purity"]) if (Process_param["Target_Purity"] - Process_specs["Purity"]) > 0 else 0 #10 million pounds per year penalty per percentage purity under target
